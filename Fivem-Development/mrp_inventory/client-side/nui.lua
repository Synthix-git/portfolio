...
CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(Ped) and Drops[Route] then
			local Coords = GetEntityCoords(Ped)
			-- apenas desenhar quando estiver realmente perto; otimizar com dist^2
			for _,v in pairs(Drops[Route]) do
				local dx,dy,dz = Coords.x - v["coords"].x, Coords.y - v["coords"].y, Coords.z - v["coords"].z
		...
		-- se nada ativou, dormir um pouco mais para reduzir CPU
		Wait(TimeDistance == 999 and 600 or TimeDistance)
	end
end)
...
-- Wrapper para remover zonas no target, compatível com várias APIs
local function RemoveTargetZone(id)
    local ok = pcall(function() exports["target"]:RemBoxZone(id) end)
    if not ok then ok = pcall(function() exports["target"]:RemoveBoxZone(id) end) end
    if not ok then ok = pcall(function() exports["target"]:RemZone(id) end) end
    if not ok then ok = pcall(function() exports["target"]:RemoveZone(id) end) end
    if not ok then
        -- Fallback por evento (alguns targets expõem assim)
        TriggerEvent("target:RemoveZone", id)
    end
end
            ...
            if #(Coords - Components[Active]["1"][1]["xyz"]) <= 1000 then
                ...
                if Objects["1"] then
                    for Number, v in pairs(Objects) do
                        if Number ~= "1" then
                            RemoveTargetZone("Helicrash:" .. Number)
                        end
...
function HeliBlip(Number)
    if Components[Number] then
        Blip = AddBlipForCoord(
            Components[Number]["1"][1]["x"],
            Components[Number]["1"][1]["y"],
            Components[Number]["1"][1]["z"]
        )

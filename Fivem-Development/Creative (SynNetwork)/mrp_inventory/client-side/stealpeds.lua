		...
		if not IsPedInAnyVehicle(Ped) and IsPedArmed(Ped,6) and IsPlayerFreeAiming(Pid) then
			...
			if Entitys and GetGameTimer() >= Timer and GetVehiclePedIsIn(Entitys,true) == 0 and not Entity(Entitys)["state"]["Steal"] then
				...
				while true do
					...
					local dx,dy,dz = Coords.x - EntityCoords.x, Coords.y - EntityCoords.y, Coords.z - EntityCoords.z
					local d2 = dx*dx + dy*dy + dz*dz
					if d2 <= (2 * 2) and IsPlayerFreeAiming(Pid) then
						Progress = Progress - 1
					...
					-- inner loop doesn't need to run every frame; 5ms is precise enough
					Wait(5)
				end
			end
		end
